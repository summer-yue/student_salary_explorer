...
.controller('bySalaryCtrl', function($scope, $http) {
    ...
    $scope.colleges_in_range = {};
    $scope.majors_in_range = {};
    //console.log("Entering by salary controller api");
    
    $scope.display_college_major_in_range = function() {
        $http.get('/api/colleges_based_on_salary_range/' + $scope.salary_range_low + '/' +
            $scope.salary_range_high).success(function(result){
            $scope.colleges_in_range = result.data;
        });

        $http.get('/api/majors_based_on_salary_range/' + $scope.salary_range_low + '/' +
            $scope.salary_range_high).success(function(result){
